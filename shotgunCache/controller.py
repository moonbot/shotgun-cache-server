...
class ShotgunCacheServer(object):
    ...
    def __init__(self, config):
        ...
        self.messageQueue = MessageQueue(self, self.config, self.workQueue)
    ...
    def start(self):
        ...
        if not len(self.entityConfigManager.configs):
            raise IOError("No entity configs found to cache in {0}".format(self.config.entityConfigFolderPath))
        ...
        self.messageQueueGreenlet = gevent.spawn(self.messageQueue.start)
...
class MessageQueue(object):
    ...
    def handle_updateCache(self):
        LOG.debug("Forcing update to cache until time")
        # TODO - EOD May 11th
        pass
